...
batchsize = 280
unlabeled_batchsize = 140
labeled_batchsize = 140
print('Graph-based')
...
class Classifier(nn.Module):
    ...
    def forward(self, embedding):
        prediction = torch.mm(embedding, self.W)
        return prediction


class MainModel(nn.Module):
    def __init__(self, encodernet, classifiernet):
        super(MainModel, self).__init__()
        self.encodernet = encodernet
        self.classifiernet = classifiernet

    def forward(self, img, label, mask):
        embedding = self.encodernet(img)
        prediction = self.classifiernet(embedding)
        ...
        similarity = torch.mul(similarity, mask)
        loss2 = torch.mean(similarity)
        return loss1, loss2


encoder = Encoder().to(device)
classifier = Classifier().to(device)
mainmodel = MainModel(encoder, classifier).to(device)
...
for epoch in range(50):
    ...
    for batch in range(max_batch):
        ...
        batch_img = np.reshape(batch_img,newshape=(batchsize,3*64*320))
        sq_dist = pdist(batch_img)
        pairwise_dists = squareform(sq_dist)
        mask = np.zeros(shape=(batchsize, batchsize))
        for i in range(unlabeled_batchsize):
            dist = list(pairwise_dists[i])
            top9 = list(map(dist.index, heapq.nsmallest(9, dist)))
            for j in range(9):
                if top9[j] >= unlabeled_batchsize:
                    mask[top9[j]][i] = 1
        for i in range(labeled_batchsize):
            dist = list(pairwise_dists[i + unlabeled_batchsize])
            top9 = list(map(dist.index, heapq.nsmallest(9, dist)))
            for j in range(9):
                if top9[j] >= unlabeled_batchsize and top9[j] != i + unlabeled_batchsize:
                    if batch_label[i] == batch_label[top9[j] - unlabeled_batchsize]:
                        mask[i + unlabeled_batchsize][top9[j]] = 1
                        mask[top9[j]][i + unlabeled_batchsize] = 1
                    else:
                        mask[i + unlabeled_batchsize][top9[j]] = -1
                        mask[top9[j]][i + unlabeled_batchsize] = -1
        batch_img = np.reshape(batch_img, newshape=(batchsize, 3, 64, 320))
        ...
        optimizer.zero_grad()
        batch_loss1, batch_loss2 = mainmodel(batch_img, batch_label, mask)
    ...
    for batch in range(max_batch):
        ...
        with torch.no_grad():
            batch_pred = classifier(encoder(batch_img))
