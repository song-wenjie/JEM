...
history_score = 0
global_step = 0
labeled_ensemble_predictions = np.zeros(shape=(len(x_train),4))
unlabeled_ensemble_predictions = np.zeros(shape=(len(x_unlabeled),4))
labeled_target_vectors = np.zeros(shape=(len(x_train),4))
unlabeled_target_vectors = np.zeros(shape=(len(x_unlabeled),4))

for epoch in range(50):
    encoder.train()
    index = np.random.permutation(np.arange(len(x_unlabeled)))
    x_unlabeled = np.array(x_unlabeled)[index]
    unlabeled_ensemble_predictions = np.array(unlabeled_ensemble_predictions)[index]
    unlabeled_target_vectors = np.array(unlabeled_target_vectors)[index]
    index = np.random.permutation(np.arange(len(x_train)))
    x_train = np.array(x_train)[index]
    y_train = np.array(y_train)[index]
    labeled_ensemble_predictions = np.array(labeled_ensemble_predictions)[index]
    labeled_target_vectors = np.array(labeled_target_vectors)[index]
    ...
    for batch in range(max_batch):
        ...
        batch_labeled_target_vectors = labeled_target_vectors[batch * labeled_batchsize:batch * labeled_batchsize + labeled_batchsize]
        batch_unlabeled_target_vectors = unlabeled_target_vectors[batch * unlabeled_batchsize:batch * unlabeled_batchsize + unlabeled_batchsize]
        batch_target_vectors = np.concatenate((batch_unlabeled_target_vectors, batch_labeled_target_vectors), axis=0)

        batch_target_vectors = torch.from_numpy(batch_target_vectors).float().to(device)

        batch_img = augment(batch_img)
        batch_img = torch.from_numpy(batch_img).float().to(device)
        optimizer.zero_grad()
        output = encoder(batch_img)

        unlabeled_output, labeled_output = torch.split(output, unlabeled_batchsize, dim=0)
        batch_loss1 = F.cross_entropy(labeled_output, batch_label)

        output = F.softmax(output, dim=1)
        batch_loss2 = get_current_consistency_weight(epoch) * F.mse_loss(output, batch_target_vectors)
        batch_loss = batch_loss1 + batch_loss2
        batch_loss.backward()
        optimizer.step()
        global_step = global_step + 1

        unlabeled_output = F.softmax(unlabeled_output, dim=1)
        unlabeled_output = unlabeled_output.detach().cpu().numpy()
        labeled_output = F.softmax(labeled_output, dim=1)
        labeled_output = labeled_output.detach().cpu().numpy()

        for i in range(unlabeled_batchsize):
            temp = unlabeled_ensemble_predictions[batch * unlabeled_batchsize + i]
            temp = 0.6 * temp + 0.4 * unlabeled_output[i]
            unlabeled_ensemble_predictions[batch * unlabeled_batchsize + i] = temp
            unlabeled_target_vectors[batch * unlabeled_batchsize + i] = temp/(1-np.power(0.6, epoch+1))

        for i in range(labeled_batchsize):
            temp = labeled_ensemble_predictions[batch * labeled_batchsize + i]
            temp = 0.6 * temp + 0.4 * labeled_output[i]
            labeled_ensemble_predictions[batch * labeled_batchsize + i] = temp
            labeled_target_vectors[batch * labeled_batchsize + i] = temp / (1 - np.power(0.6, epoch + 1))
