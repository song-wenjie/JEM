...
def d_entropy1(y):
    y1=torch.sum(y,axis=0)/batchsize
    y2 = torch.sum(-y1 * torch.log(y1))
    return y2


def d_entropy2(y):
    y1=-y*torch.log(y)
    y2=torch.sum(y1)/batchsize
    return y2
...
for epoch in range(50):
    ...
    for batch in range(max_batch):
        ...
        D_real = netD(batch_img)
        noise = torch.randn(batchsize, 960).to(device)
        fake = netG(noise)
        D_fake = netD(fake)

        D_real_unlabeled, D_real_labeled = torch.split(D_real, unlabeled_batchsize, dim=0)
        log_D_real_labeled = torch.log(D_real_labeled)
        ce_loss = F.nll_loss(input=log_D_real_labeled, target=batch_label)

        L_dis = -1 * (d_entropy1(D_real) - d_entropy2(D_real) + d_entropy2(D_fake)) + weight*ce_loss
        ...
        L_gen = -d_entropy1(D_fake) + d_entropy2(D_fake)
